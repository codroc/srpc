#ifndef SRPC_PROTO_HTTP_H
#define SRPC_PROTO_HTTP_H
...
private:
    static std::pair<std::string, std::string> get_pair(const std::string& line);
private:
    MapType _headers;
};

class HttpBody {
    // brief: body 中可以存储任意格式的数据，但要在 header 的 Content-Type 中表明用了哪种格式
    // 这里我打算支持以下几种格式:
    // 1. plain text
    // 2. google protobuf
    // 3. json
...
public:
    ...
    HttpHeader& get_header() { return _header; }
    const HttpHeader& get_header() const { return _header; }
